...
SINGLE_ORDER_LATENCY_TRIALS="${SINGLE_ORDER_LATENCY_TRIALS:-5}"
SINGLE_ORDER_LATENCY_SAMPLES="${SINGLE_ORDER_LATENCY_SAMPLES:-1000000}"
SINGLE_ORDER_LATENCY_WARMUP="${SINGLE_ORDER_LATENCY_WARMUP:-10000}"
PIN_CPU="${PIN_CPU:-0}"
...
if should_run single_order_latency; then
    run_pinned "$BUILD_DIR/single_order_latency_benchmark" \
        --output-dir="$OUTPUT_DIR" \
        --samples="$SINGLE_ORDER_LATENCY_SAMPLES" \
        --warmup="$SINGLE_ORDER_LATENCY_WARMUP" \
        --trials="$SINGLE_ORDER_LATENCY_TRIALS"
fi
